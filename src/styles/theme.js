...
export const createStyles = (viewport) => {
  ...
  navInner: { maxWidth: 1200, margin: '0 auto', padding: isMobile ? '12px 16px' : '0 24px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', gap: isMobile ? 12 : 24, minHeight: isMobile ? 'auto' : 68 },
  logo: { display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', cursor: 'pointer', justifyContent: isMobile ? 'center' : 'flex-start' },
  ...
  navLinks: { display: 'flex', gap: 6, alignItems: 'center', overflowX: isMobile ? 'auto' : 'visible', paddingBottom: isMobile ? 4 : 0, width: isMobile ? '100%' : 'auto' },
  ...
  signInBtn: { padding: '8px 20px', borderRadius: 8, border: 'none', background: theme.colors.leaf, color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', width: isMobile ? '100%' : 'auto' },
  navActions: { display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 10, alignItems: 'center', width: isMobile ? '100%' : 'auto' },
  userActions: { display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, alignItems: 'center', width: isMobile ? '100%' : 'auto' },
